...
var Turtle = function Turtle(config) {
	...
	this.d = typeof config.d !== 'undefined' ? config.d : 10;
	this.theta = typeof config.theta !== 'undefined' ? config.theta : 90;
	this.productions = typeof config.theta !== 'undefined' ? config.productions : {};
		...
		for (var i = 0; i < axiom.length; i++) {
			...
			if (this.productions[token]) { // check if the token is in the production list
				if (typeof this.productions[token] === 'object') { // check if this production is stochastic
					// check to see the probabilities add up to 1.0
					...
					if (probabilityTotal === 1) { // confirm the probabilities actually add up to one
					...
					} else { // If the probabilities don't add up to 1.0, exit
						console.warn('Probabilities added up to ' + probabilityTotal);
					}
