...
function resetSession(settings) {
	...
	SOM.geometries = [];
	SOM.cameras = {}
	SOM.cameras.main = new THREE.PerspectiveCamera(30, 1, 1, 1000000);
	SOM.cameras.x = {}; // TODO
	SOM.cameras.y = {};
	SOM.cameras.z = {};
	session.settings.cameraPosition = new THREE.Vector3(100, 100, 100);
    SOM.cameras.main.position.copy(session.settings.cameraPosition);


	session.cameraControls = new OrbitControls(SOM.cameras.main, session.renderer.domElement);
	session.cameraControls.target.set(0, 0, 0);
}

function resizeCamera(width, height) {
	session.renderer.setSize( width, height );
	SOM.cameras.main.aspect = width / height;
	SOM.cameras.main.updateProjectionMatrix();
}
...
function render () {
	...
	session.renderer.render(session.scene, SOM.cameras.main);
